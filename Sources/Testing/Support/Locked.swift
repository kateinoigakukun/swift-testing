...
typealias Locked<T> = CustomLocked<pthread_mutex_t>
#elseif os(WASI) && compiler(>=6.1) && !_runtime(_multithreaded)
// No locks on WASI without multithreaded runtime.
typealias Locked<T> = CustomLocked<Never>
#else
typealias Locked<T> = CustomLocked<Mutex<Void>>
#endif
